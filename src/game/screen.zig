...
pub const Voxels = struct {
    ...
    pub fn deinit(self: *@This(), ctx: platform.Context) void {
        self.alloc.free(self.pos_size);
        self.alloc.free(self.col);
    }

    pub fn addVoxel(self: *@This(), pos: Vec3f, size: f32, col: [3]u8) void {
        const index = self.count * 4;
        self.pos_size[index] = pos.items[0];
        self.pos_size[index+1] = pos.items[1];
        self.pos_size[index+2] = pos.items[2];
        self.pos_size[index+3] = size;

        const cindex = self.count * 3;
        self.col[cindex] = col[0];
        self.col[cindex+1] = col[1];
        self.col[cindex+2] = col[2];
...
pub const Screen = struct {
    ...
    pub fn init(self: *@This(), ctx: platform.Context) !void {
        ...
        self.voxels.addVoxel(Vec3f.new(.{1, 0, 0}), 1, .{255, 0, 0});
        self.voxels.addVoxel(Vec3f.new(.{-1, 0, 0}), 1, .{255, 0, 0});
        self.voxels.addVoxel(Vec3f.new(.{0, 0, 1}), 1, .{255, 0, 0});
        self.voxels.addVoxel(Vec3f.new(.{0, 0, -1}), 1, .{255, 0, 0});
    ...
    pub fn render_InstancedCube_shader(self: *@This(), ctx: platform.Context, alpha: f64) !void {
        ...
        c.glDrawArraysInstanced(c.GL_TRIANGLE_STRIP, 0, @divFloor(VERTS.len, 3), @intCast(c_int, self.voxels.count));
    }
    ...
    pub fn render_Raybox_shader(self: *@This(), ctx: platform.Context, alpha: f64) !void {
        ...
        c.glDrawArraysInstanced(c.GL_TRIANGLE_STRIP, 0, @divFloor(QUAD_VERTS.len, 2), @intCast(c_int, self.voxels.count));
    }
};
